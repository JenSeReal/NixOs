...
in {
  options.JenSeReal.programs.cli.git = with types; {
    ...
    signingKey = mkOpt str "${config.home.homeDirectory}/.ssh/id_ed25519"
  ...
  config = mkIf cfg.enable {
    programs.git = {
      ...
      extraConfig = {
        ...
        fetch = { prune = true; };
        safe = { directory = "${config.home.homeDirectory}/dev"; };
        gpg = { format = "ssh"; };
      };
      ignores = [
        ".DS_Store"
